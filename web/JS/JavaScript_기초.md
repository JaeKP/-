...
1. window: DOM을 표현하는 창 (브라우저 탭)로 최상위 객체이다 (작성 시 생략이 가능하다.)
...
- 실제 데이터는 따로 있고 참조값을 저장하는 것이 Reference Type이다
...
연산할 때 발생한다. (오류를 발생시키지 않고 유연하게 타입을 바꿔버린다..)
...
- Number로 형 변환
  ...
  `+`연산에서 피연산자 중 String이 없으면 숫자형 연산이 된다.  (true는 1, false와 null은 0, undefined는 NaN이다.)

  이외의 모든 산술연산은 숫자형 연산이다. 

  관계 연산자도 마찬가지이다. (true는 1, false와 null은 0, undefined는 NaN이다.)

  만약 문자열간의 관계 연산이면 유니코드 숫자를 통해 연산한다.
...
switch(num){
    ...
    case 1:
        console.log(2)                     // 2 
    default:
        console.log('0, 1, 2가 아닌 경우')  // 0, 1, 2가 아닌 경우
}       
```
...
- 파이썬과 달리 while문처럼 조건 식이 있어야 한다.  
  - while문과 똑같이 동작한다.
  - **조건식: (처음 for문을 시작할 때 딱 한 번만 한다 ; 매 반복 시행 전 평가되는 부분; 매 반복 시행 이후 평가되는 부분)**
  - 복잡한 수식을 사용할때는 별로 ... ㅎㅎ 
...
- 객체를 순회할 때 사용한다. 
  - 객체를 순회하면 `속성(key)`를 가져온다. 

- 배열도 순회 가능하지만 권장하지 않는다. (배열의 index를 순회하기 때문에..)
